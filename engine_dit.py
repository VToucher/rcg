...
def gen_img(model, model_without_ddp, vae, diffusion, ema_params, rdm_sampler, args, epoch, batch_size=16, log_writer=None, use_ema=True, cfg=1.0, cond_idx=0):
    model.eval()
    num_steps = args.num_images // (batch_size * misc.get_world_size()) + 1
    save_folder = os.path.join(args.output_dir, "step{}_cfg{}_mgscale_{}".format(args.num_sampling_steps, cfg, rdm_sampler.mg_kwargs['mg_scale']))
    if misc.get_rank() == 0:
